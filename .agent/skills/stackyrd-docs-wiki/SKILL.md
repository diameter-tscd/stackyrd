---
name: stackyrd-nano-docs-wiki
description: >
  Maintain and update the stackyrd-nano project's hand-written documentation in
  docs_wiki/. Use this skill whenever the user adds, modifies, or removes
   packages, middleware, services, or infrastructure components, and
  ...
  or "docs" in the context of the stackyrd-nano project, asks to "keep docs in
...
# stackyrd-nano Documentation Wiki Guide
...
1. **Code-driven updates** — when a service, middleware, infrastructure
   component, or utility package is added, changed, or removed, the
...
- Always use fenced code blocks with language tags: ` ```go `, ` ```yaml `,
  ` ```bash `, ` ```json `, ` ```mermaid `.
- Go code examples must use the module path `stackyrd-nano` for imports.
...
1. **Identify the doc file** — check if a file already exists for the topic
   (e.g., a new `pkg/cache/` package maps to `CACHING.md`). If not, you'll
   need to create one.
...
5. **Update `docs_wiki/README.md`** — add/remove/modify the table entry.
   - If adding a new doc to the "Package Deep Dives" section, include the
      package path in the table (e.g., `pkg/cache/`).
...
- **Don't include auto-generated Swagger docs** — the nano version does not
  include Swagger; use manual API documentation in docs_wiki instead.
