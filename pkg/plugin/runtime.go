...
func (r *ScriptRuntime) Execute(ctx Context, scriptPath string, args map[string]interface{}) (*Result, error) {
	...
	vm.Set("$args", args)
	vm.Set("$limits", map[string]int64{
		"max_timeout_ms":   ctx.Limits.MaxTimeoutMs,
		"max_memory_bytes": ctx.Limits.MaxMemoryBytes,
	})

	loggerObj := vm.NewObject()
	loggerObj.Set("info", func(msg string) { ctx.Logger.Info(msg, "plugin", ctx.ID) })
	loggerObj.Set("warn", func(msg string) { ctx.Logger.Warn(msg, "plugin", ctx.ID) })
	loggerObj.Set("error", func(msg string) { ctx.Logger.Error(msg, nil, "plugin", ctx.ID) })
	loggerObj.Set("debug", func(msg string) { ctx.Logger.Debug(msg, "plugin", ctx.ID) })
	vm.Set("$logger", loggerObj)

	infraObj := vm.NewObject()
	infraObj.Set("get", func(name string) interface{} {
